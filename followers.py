...
class Followers(object):
    ...
    def find_followings(self, buttons):
        ...
        self.following_button = [
            button for button in buttons if 'following' in button.get_attribute('href')]
        self.following_button[0].click()
        time.sleep(2)

        def followingsList(): return driver.find_element_by_css_selector(
            'div[role=\'dialog\'] ul')
        numberOfFollowingsInList = len(
            followingsList().find_elements_by_css_selector('li'))
        def following_number(): return driver.find_element_by_xpath(
    ...
    def find_followers(self, buttons):
        ...
        follower_button = [
            button for button in buttons if 'followers' in button.get_attribute('href')]
        follower_button[0].click()
        time.sleep(2)

        def followersList(): return driver.find_element_by_css_selector(
            'div[role=\'dialog\'] ul')
        numberOfFollowersInList = len(
            followersList().find_elements_by_css_selector('li'))
        def follower_number(): return driver.find_element_by_xpath(
        ...
        def users(): return followersList().find_elements_by_css_selector('li')

        for user in users():
            def userLink(): return user.find_element_by_css_selector('a')
    ...
    def find_target_users(self):
        ...
        time.sleep(2)
        self.compare_following_and_followers(
            self.follower_accounts, self.following_accounts)
    ...
    def unfollow_by_userlink(self, userlink):
        ...
        ActionChains(self.driver).move_to_element(
            follow_button).click().perform()
        time.sleep(2)

        button = self.driver.find_element_by_xpath(
            "//button[@class='aOOlW -Cab_   ']")
        click(driver, button)
